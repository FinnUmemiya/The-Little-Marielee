...
function openModal() {
  ...
  if (currentWhitelistCount >= maxSupply) {
...
async function handleSubmit(e) {
  ...
  const maxSupply = getNumericSupply();
  if (currentWhitelistCount >= maxSupply && localStorage.getItem("marielee_whitelisted") !== "true") {
...
function loadStoredSettings() {
  ...
  if (stored) {
    try {
      applyProjectSettings(JSON.parse(stored));
    } catch (e) {}
  }
}
...
function applyProjectSettings(settings) {
  ...
  const supply = (projectSettings.totalSupply || "2,222").trim();
...
function getNumericSupply() {
  const raw = String(projectSettings.totalSupply || "2222").replace(/[^0-9]/g, "");
  const num = parseInt(raw, 10);
  return isNaN(num) || num <= 0 ? 2222 : num;
}
...
function checkSupplyCap() {
  ...
  const maxSupply = getNumericSupply();
  const isFull = currentWhitelistCount >= maxSupply;
