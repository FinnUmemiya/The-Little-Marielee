...
function doPost(e) {
  try {
    ...
    if (action === "submit") {
      ...
      const rawSupply = String(currentSettings.totalSupply || "0").replace(/[^0-9]/g, "");
...
function saveProjectSettings(newSettings) {
  ...
  const updated = {
    totalSupply: (newSettings.totalSupply !== undefined && newSettings.totalSupply !== null && String(newSettings.totalSupply).trim() !== "" ? newSettings.totalSupply : current.totalSupply || "0").toString().trim(),
